...
for idx in indexes:
    print(f"Index: {idx.name}, Status: {idx.status}, Docs: {idx.document_count}")
```

Replace `YOUR_PROJECT_ENDPOINT` with the endpoint shown in your project overview.
...
> ⚠️ **Vignette: Quota exceeded error**
...
```bash
curl -X POST "https://YOUR_PROJECT_ENDPOINT/chat/completions" \
...
> If the model answers from general knowledge instead of your docs, check that **Add your data** is toggled ON in the playground. In API calls, verify the `data_sources` block is included — without it, the model ignores the index.
...
if hasattr(response.choices[0].message, 'context'):
    ...
    for c in citations:
        print(f"📄 Source: {c['title']} — {c['filepath']}")
```
...
## Executive Takeaway (for exec participants)
What you just saw: TVA's compliance documents are now searchable by AI in plain English. An engineer asking "what are our patch requirements?" gets an instant, cited answer instead of searching through 200-page PDFs. This is the foundation everything else builds on.
